...
## Shelter data generation

- Shelter data is generated by SvelteKit from `src/routes/shelters.json/+server.ts`
- To generate the public JSON artifact locally, run `npm run build`
- Generated artifact path: `build/shelters.json`
- Only shelters with `Verification Status` = `Open` are included
- Internal/admin fields are stripped from the output JSON
